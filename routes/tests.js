...
router.post('/', auth, authorize('master_admin'), [
  ...
  body('testType').optional().isIn(['Assessment', 'Practice', 'Mock Test', 'Specific Company Test']).withMessage('Invalid test type'),
  ...
  body('marksPerQuestion').optional().custom((value, { req }) => {
    if (req.body.hasSections === true || req.body.hasSections === 'true') return true;
    if (req.body.hasCodingSection === true || req.body.hasCodingSection === 'true') return true;
...
// Get students already assigned to a test assignment (College Admin)
router.get('/assignment/:id/assigned-students', auth, authorize('college_admin'), async (req, res) => {
  try {
    const { id } = req.params;

    // Find the test assignment and its related student assignments
    const assignment = await TestAssignment.findOne({
      _id: id,
      collegeId: req.user.collegeId
    });
    ...
    // Find all student assignments for this test
    const studentAssignments = await TestAssignment.find({
...
router.post('/:id/start', auth, authorize('student'), async (req, res) => {
  try {
    ...
    // Add coding questions if enabled (for all test types including Assessment, Mock Test, etc.)
    console.log('🔍 Checking for coding questions...', {
      hasCodingSection: test.hasCodingSection,
      codingQuestionsLength: test.codingQuestions?.length,
      testType: test.testType,
      testId: test._id
    });
    
    if (test.hasCodingSection && test.codingQuestions && test.codingQuestions.length > 0) {
      console.log('✅ Adding', test.codingQuestions.length, 'coding questions to response');
      ...
      console.log('❌ No coding questions found for this test');
    }

    console.log('📤 Sending test response to student:', {
      testId: testForStudent._id,
      hasCodingSection: testForStudent.hasCodingSection,
      codingQuestionsInResponse: testForStudent.codingQuestions?.length || 0,
      numberOfQuestions: testForStudent.numberOfQuestions
    });
...
// Save test progress (allow resuming later)
router.post('/:id/save-progress', auth, authorize('student'), [
  body('answers').isArray(),
  body('startTime').isISO8601(),
  body('currentProgress').isInt({ min: 0 }),
  body('violations').optional().isInt({ min: 0 })
], async (req, res) => {
  try {
    const { answers, startTime, currentProgress, violations = 0 } = req.body;
    const testId = req.params.id;
    ...
    if (existingAttempt && ['completed', 'submitted', 'timeout', 'auto-submitted-violations'].includes(existingAttempt.status)) {
      return res.status(400).json({ error: 'Test already submitted' });
    }

    // Save progress to localStorage equivalent on backend (using a cache collection if needed)
    // For now, we'll store in session or return success to let client handle storage
    logger.info('Test progress saved', { testId, studentId: req.user._id, progress: currentProgress, violations });
    ...
    logger.errorLog(error, { context: 'Save test progress error' });
    res.status(500).json({ error: 'Server error' });
  }
});

// Resume test - Get saved progress
router.get('/:id/resume', auth, authorize('student'), async (req, res) => {
  try {
    const testId = req.params.id;
    ...
    if (existingAttempt) {
      return res.status(400).json({ 
        ...
        results: existingAttempt
      });
    }

    // Return test for resuming (same as start test)
    ...
    res.json({
      message: 'Test ready to resume',
      test: testForStudent,
      canResume: true,
      startTime: now
    });
  } catch (error) {
    logger.errorLog(error, { context: 'Resume test error' });
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.post('/:id/submit', auth, authorize('student'), [
  ...
  try {
    ...
    if (violations >= 3) {
      ...
      message = `Test auto-submitted due to ${violations} violations (fullscreen exit detected). Please maintain test environment.`;
    }
