...
router.post('/users', auth, authorize('college_admin'), [
  ...
  try {
    ...
    logger.errorLog(error, { context: 'Create user error' });
...
router.delete('/users/:userId', auth, authorize('college_admin'), async (req, res) => {
  try {
    const { userId } = req.params;
    console.log('[DELETE /users/:userId] Starting delete operation', { userId, collegeId: req.user.collegeId });

    if (!userId) {
      console.log('[DELETE /users/:userId] No userId provided');
      return res.status(400).json({ error: 'User ID is required' });
    }

    // Validate userId is a valid MongoDB ID
    if (!userId.match(/^[0-9a-fA-F]{24}$/)) {
      console.log('[DELETE /users/:userId] Invalid userId format', { userId });
    ...
    console.log('[DELETE /users/:userId] User lookup result', { 
      found: !!user, 
      userId,
      userRole: user?.role,
      userCollegeId: user?.collegeId,
      adminCollegeId: req.user.collegeId 
    });

    if (!user) {
      console.log('[DELETE /users/:userId] User not found', { userId });
      return res.status(404).json({ error: 'User not found' });
    }

    // Prevent deletion of college admin
    if (user.role === 'college_admin') {
      console.log('[DELETE /users/:userId] Cannot delete college admin', { userId });
      return res.status(403).json({ error: 'Cannot delete college admin' });
    }

    // Verify college ownership: user must either have matching collegeId or no collegeId (legacy)
    if (user.collegeId) {
      const userCollegeIdStr = user.collegeId.toString();
      // req.user.collegeId is a populated College object, so access its _id
      const adminCollegeIdStr = (req.user.collegeId._id || req.user.collegeId).toString();
      
      if (userCollegeIdStr !== adminCollegeIdStr) {
        console.log('[DELETE /users/:userId] User belongs to different college', { 
          userCollegeId: userCollegeIdStr,
          adminCollegeId: adminCollegeIdStr
        });
        return res.status(403).json({ error: 'Cannot delete user from another college' });
      }
    }
    // If user has no collegeId, allow deletion (legacy data)

    console.log('[DELETE /users/:userId] Deleting user', { userId, userRole: user.role });
    const deletedUser = await User.findByIdAndDelete(userId);
    
    if (!deletedUser) {
      console.log('[DELETE /users/:userId] Failed to delete user', { userId });
      return res.status(500).json({ error: 'Failed to delete user' });
    }

    console.log('[DELETE /users/:userId] User deleted successfully', { userId, deletedUserRole: deletedUser.role });
    ...
    try {
      ...
      if (college) {
        await college.updateStats();
        console.log('[DELETE /users/:userId] College stats updated', { collegeId: req.user.collegeId });
      }
    } catch (statError) {
      console.warn('[DELETE /users/:userId] Error updating college stats', { error: statError.message });
    ...
    console.error('[DELETE /users/:userId] Unexpected error', { error: error.message, stack: error.stack });
...
router.get('/hierarchy', auth, authorize('college_admin', 'faculty'), async (req, res) => {
  try {
    ...
    logger.errorLog(error, { context: 'Get hierarchy error' });
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.get('/students/filtered', auth, authorize('college_admin', 'faculty'), async (req, res) => {
  try {
    ...
    logger.errorLog(error, { context: 'Get filtered students error' });
    res.status(500).json({ error: 'Server error' });
  }
});
