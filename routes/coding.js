...
const { executeCodeSafely } = require('../utils/sandbox');
...
const codeExecutionLimiter = rateLimit({
  ...
  max: 10,                     // Max 10 executions per minute per IP
...
router.post('/questions', authenticateToken, validateQuestionInput, async (req, res) => {
  try {
...
router.put('/questions/:id', authenticateToken, validateQuestionInput, async (req, res) => {
  try {
    const { id } = req.params;
    const { testCases, ...questionData } = req.body;
...
router.post('/run', authenticateToken, codeExecutionLimiter, validateCodingSubmission, async (req, res) => {
  try {
    ...
    try {
      ...
      for (let i = 0; i < testCases.length; i++) {
        ...
        // Simple pass/fail check - output matching
        testResults.push({
          test_case_number: i + 1,
          passed: result.output.includes(testCase.expected_output.trim()),
      ...
      res.status(400).json({ error: execError.message });
    }
  } catch (error) {
...
router.post('/submit', authenticateToken, codeExecutionLimiter, validateCodingSubmission, async (req, res) => {
  try {
    ...
    try {
      ...
      for (let i = 0; i < testCases.length; i++) {
        const testCase = testCases[i];
        const passed = result.output.includes(testCase.expected_output.trim());
      ...
      const totalWeight = testCases.reduce((sum, tc) => sum + (tc.weight || 0), 0);
      const earnedWeight = testResults
        .filter(r => r.passed)
        .reduce((sum, r) => {
      ...
      if (testCasesPassed === totalTestCases) {
        status = 'accepted';
      } else if (result.executionTime > 4000) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      if (language === 'javascript') {
        // Use isolated-vm if available, otherwise fall back to built-in vm
        if (ivm) {
          ...
          // Fallback to built-in vm module
          const outputLogs = [];
          const inputLines = input.split('\n');
          let inputIndex = 0;
